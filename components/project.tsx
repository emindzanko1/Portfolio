...
export default function Project({ title, description, tags, imageUrl }: ProjectProps) {
  ...
  function capitalizeExcept(str: string, exceptions: string[]): string {
    return str
      .split(' ')
      .map(word => exceptions.includes(word.toLowerCase()) ? word : word.charAt(0).toUpperCase() + word.slice(1))
      .join(' ');
  }
    ...
    border border-black/5 rounded-lg overflow-hidden sm:pr-8 relative sm:h-[20rem]
     hover:bg-gray-200 transition sm:group-even:pl-8 dark:text-white dark:bg-white/10 dark:hover:bg-white/20'
      >
        ...
        >
          <h3 className='font-semibold capitalize'>{capitalizeExcept(title, ['at'])}</h3>
